...
function normalizeTicket(ticket: Partial<LocalSupportTicket>): LocalSupportTicket {
  ...
  return {
    ...
    currentCategory: ticket.currentCategory || category,
    currentProblemName: ticket.currentProblemName || ticket.issueType || '',
    currentProductId: ticket.currentProductId || '',
    currentProductName: ticket.currentProductName || '',
    currentModel: ticket.currentModel || ticket.productModel || '',
    currentHardwareVersion: ticket.currentHardwareVersion || '',
    currentSN: ticket.currentSN || ticket.serialNumber || '',
    activeSupportFlow: ticket.activeSupportFlow || '',
    activeProblemId: ticket.activeProblemId || ticket.issueType || '',
    activeSolutionId: ticket.activeSolutionId || '',
    activeStepGroupId: ticket.activeStepGroupId || '',
    activeProcedureId: ticket.activeProcedureId || '',
    waitingForGuidedConfirmation: Boolean(ticket.waitingForGuidedConfirmation),
    waitingForModelOrSticker: Boolean(ticket.waitingForModelOrSticker),
    waitingForProblemDetails: Boolean(ticket.waitingForProblemDetails),
    waitingForLocation: Boolean(ticket.waitingForLocation),
    productInfoAsked: Boolean(ticket.productInfoAsked),
    ...
    awaitingLocation: Boolean(ticket.awaitingLocation),
    escalationActive: Boolean(ticket.escalationActive),
    escalationCompleted: Boolean(ticket.escalationCompleted),
    category,
    issue: ticket.issue || '',
